...
{
  std::cout << "Called RNAPolymerase kernel\n";
...
{
  std::cout << "Called RibosomeComplex kernel\n";
...
template <>
void KernelRunHelper::Kernel<Flagellum>::run(WorldState* world, Protein* p)
{
  Flagellum& flagellum = *reinterpret_cast<Flagellum*>(p);
  if (!flagellum.self_eaten) {
    ++world->n_flagella;
    flagellum.EatYourself();
  }
}

void Flagellum::FetchKernelRunner(KernelRunHelper** kernel_runner)
{
  *kernel_runner = new KernelRunHelper();
  (*kernel_runner)->create_kernel<Flagellum>();
}
